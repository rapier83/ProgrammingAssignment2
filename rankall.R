...
rankall <- function(outcome, num="best") {
    ...
    tmp.x <- lapply(tmp.x, 
                    function(x) 
                        cbind(x[1:3],rank = rank(x[,3],ties.method="first"))
                    )
    
    if (is.numeric(num) == TRUE) {
        tmp.rank <- lapply(tmp.x, function(x) x[x$rank == num, ])
    }
    else {
        if (num == "best") {
            tmp.rank <- lapply(tmp.x, function(x) x[x$rank == 1, ])
        }
        if (num == "worst"){
            tmp.rank <- lapply(tmp.x, function(x) x[x$rank == max(x$rank), ])
        }
    }
